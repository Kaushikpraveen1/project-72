  ...
  handleTransaction = async () => {
    ...
    db.collection('bicycles')
      ...
      .then((doc) => {
        ...
        if (bike.is_bike_available) {
          ...
          var { bikeId, userId } = this.state;
    await this.getBikeDetails(bikeId);
    await this.getuserDetails(useId);
    ...
    db.collection("bikes")
      ...
      .then(doc => {
        var bike = doc.data();
        if (bike.is_bike_available) {
          var { bikename, userName } = this.state;
          this.initiateBikeIssue(bikeId, userId, bikeName, userName);

          // For Android users only
          ToastAndroid.show("Bike issued to the user!", ToastAndroid.SHORT);

          // Alert.alert("Bike issued to the user!");
        } else {
          var { bikeName, userName } = this.state;
          this.initiateBikeReturn(bikeId, userId, bikeName, userName);
